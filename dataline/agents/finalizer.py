...
def _build_sections(state: AnalysisState) -> list[Section]:
    ...
    if state.full_step_details:
        ...
        # Inject column structure from save_result() when available.
        struct_cols = _structured_column_names(last.result.structured_json)
        if struct_cols:
            col_list = ", ".join(struct_cols)
            sections.append(Section(
                "structured_column_hint",
                f"The code's save_result() declared these columns: {col_list}\n"
                f"Preserve this column structure in the output.",
                priority=85, compressible=False,
                heading="## Column Structure (from save_result)",
            ))
