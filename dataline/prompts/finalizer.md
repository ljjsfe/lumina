...
16. PRESERVE COLUMN SEPARATION — CRITICAL: if the source data or intermediate computation produces values in separate columns (e.g., "home_score" and "away_score", "first_name" and "last_name"), keep them as separate columns in the output. Do NOT merge or concatenate values from different columns into a single column with a delimiter (e.g., do NOT output "1-1" when the answer is home=1, away=1; do NOT output "sacha harrison" when the answer is first_name="sacha", last_name="harrison").
